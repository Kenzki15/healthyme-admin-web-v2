    ...
    public function index()
    {
        try {
            ...
            $reportData = [
                ...
                'activeUsers' => 0, // Placeholder - implement as needed
            ...
            // Prepare for chart.js (labels and data arrays)
            $reportData['popularFoodCategories'] = [
                'labels' => array_keys($foodCategoryCounts),
                'data' => array_values($foodCategoryCounts)
            ];

            // Set Food Logs count to food detections count (actual user activity)
            $reportData['totalMealsLogged'] = count($foodDetections);

            // Fetch food nutrition data and get total count for Nutrition Database
            // Get the count directly from Back4App by fetching first page with limit 1 to get total count
            $foodNutritionData = $this->back4AppService->fetchFoodNutrition(1, 1);
            $reportData['foodNutritionListCount'] = $foodNutritionData['count'] ?? 0;

            return Inertia::render('Reports', [
                'reportData' => $reportData
            ]);
        } catch (\Exception $e) {
            \Log::error('Error fetching reports data', [
                'error' => $e->getMessage()
            ]);
